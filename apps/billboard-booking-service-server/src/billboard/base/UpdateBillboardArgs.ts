/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BillboardWhereUniqueInput } from "./BillboardWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { BillboardUpdateInput } from "./BillboardUpdateInput";

@ArgsType()
class UpdateBillboardArgs {
  @ApiProperty({
    required: true,
    type: () => BillboardWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => BillboardWhereUniqueInput)
  @Field(() => BillboardWhereUniqueInput, { nullable: false })
  where!: BillboardWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => BillboardUpdateInput,
  })
  @ValidateNested()
  @Type(() => BillboardUpdateInput)
  @Field(() => BillboardUpdateInput, { nullable: false })
  data!: BillboardUpdateInput;
}

export { UpdateBillboardArgs as UpdateBillboardArgs };
